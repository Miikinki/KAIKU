 ...
 * It enforces the 48h lifespan and -5 score threshold.
 */
export const fetchMessages = async (onlyRoot: boolean = true): Promise<ChatMessage[]> => {
  const cutoffTime = new Date(Date.now() - MESSAGE_LIFESPAN_MS).toISOString();
  ...
  if (isSupabaseConfigured() && supabase) {
    ...
    let query = supabase
      .from('kaiku_posts')
      .select('*')
      .gt('created_at', cutoffTime) // Enforce 48h lifespan DB-side query
      .gt('score', SCORE_THRESHOLD_HIDE) // Enforce score threshold
...
export const saveMessage = async (text: string, lat: number, lng: number, parentId?: string): Promise<ChatMessage> => {
  ...
  if (isSupabaseConfigured() && supabase) {
    const { error } = await supabase
      .from('kaiku_posts')
      .insert([{
        // ID is auto-generated by DB usually, but we can pass it if we want consistency
        // text, latitude, longitude, city_name, session_id, parent_post_id
